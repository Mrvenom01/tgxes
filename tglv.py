...
class TelegramManager:
    ...
    def cleanup_session(self):
        """Clean up session files to prevent database locked errors"""
        try:
            session_files = [
                f"{self.session_file}.session",
                f"{self.session_file}.session-journal",
                f"{self.session_file}.session-wal"
            ]
    ...
    async def start(self):
        """Initialize and start the Telegram client with database lock handling"""
        if not self.client:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "database is locked" in error_msg or "database disk image is malformed" in error_msg:
                    print(f"🔒 Database locked error detected on attempt {attempt + 1}")
                    
                    if attempt < max_retries - 1:
                        print("🔧 Attempting to fix by cleaning session files...")
                        ...
                        print("❌ Could not resolve database lock after multiple attempts")
                        print("💡 Try manually deleting all .session files and restart")
                        raise e
    ...
    async def run(self):
        ...
        try:
            if os.path.exists(f"{self.session_file}.session"):
                ...
                clean_session = input("🧹 Clean session files to prevent database lock? (Y/n): ").strip().lower()
                if clean_session != 'n':
                    self.cleanup_session()
            ...
            if "database is locked" in error_msg:
                print(f"\n❌ Database lock error: {e}")
                print("💡 SOLUTION: Delete all .session files and restart the script")
                print("🔧 Or run the script again and choose 'Y' to clean session files")
            else:
                print(f"\n❌ Unexpected error: {e}")
        finally:
            if self.client:
